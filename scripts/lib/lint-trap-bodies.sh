#!/usr/bin/env bash
# D4: trap body invariant lint — detect variable interpolation inside trap bodies.
# Usage: bash lint-trap-bodies.sh [DIRECTORY]
...
_lint_file() {
  ...
  while IFS= read -r _line; do
    ...
    if printf '%s' "$_line" | grep -qE 'trap[[:space:]]+"[^"]*\$[A-Za-z_{]'; then
      echo "VIOLATION: ${_file}:${_lineno}: trap body contains variable interpolation (double-quoted \$VAR — expanded at trap-set time):"
      echo "  $_line"
...
if [[ "$_VIOLATIONS" -gt 0 ]]; then
  echo "[lint-trap-bodies] FAIL: ${_VIOLATIONS} violation(s) found" >&2
  exit 1
fi
echo "[lint-trap-bodies] OK: no trap body interpolation violations found" >&2
exit 0
