...
_validated_ceiling() {
  local c="$1"
  case "$c" in
    ''|*[!0-9]*) c=$_DEFAULT_CEILING ;;
  esac
  [ "$c" -lt 2 ] && c=$_DEFAULT_CEILING
  echo "$c"
}
...
# Returns 2 on jq failure (propagates to caller's rc=2 [BLOCKED:harness] sidecar: corrupt-check branch).
_get_run_ordinal() {
...
_record_loop_state() {
  ...
  local scope; scope=$(_scope_of "$current_phase" "$agent")
  local ceiling; ceiling=$(_validated_ceiling "${CLAUDE_CRITIC_LOOP_CEILING:-$_DEFAULT_CEILING}")
