...
if [ -n "${PROJECT_DIR:-}" ] && \
  ...
  _state_marker="[BLOCKED:env] preflight: state-tracked — runtime state is committed; run: git ls-files plans/ | grep '\\.state/' | xargs git rm -r --cached && echo 'plans/**/*.state/' >> .gitignore && git commit -m 'chore: untrack plan state'"
  _blocked=1
  if [ -n "$_active_plan" ] && [ -f "$_active_plan" ]; then
