...
**critic-quality (B-session shell path)**: verdict is recorded by `record-verdict-direct` after `aggregate_angle_verdicts` produces the merged log; same checkpoint as other shell-driven critics.
...
```
For **`critic-feature`** (B-session only): immediately re-run `Skill("{agent}", "{prompt}")` once within this session before exiting. This avoids a wasted loop iteration when citations were hallucinated. If the re-run produces FAIL again with all citations still absent, treat as ACCEPT-OVERRIDE again and exit without a second re-run. For **shell-driven critics** (`critic-code`, `critic-spec`, `critic-test`, `critic-cross`, `critic-quality`): do NOT re-run via Skill — just exit; the outer shell loop (`run-critic-loop.sh`) will re-run the Codex reviewer in the next iteration automatically.
Exit this session immediately after the (optional) re-run — do **not** apply step 8 (FAIL-path) fixes. For **all phase-gate critics** (`critic-code`, `critic-spec`, `critic-test`, `critic-cross`, `critic-quality`, `critic-feature`): the sidecar retains the original FAIL verdict; the shell loop re-runs and should produce PASS (citations were absent). Do not look at `## Critic Verdicts` to determine the branch after ACCEPT-OVERRIDE — the explicit instruction to exit (without fix) overrides the mechanical §Skill branching logic step 8. ACCEPT-OVERRIDE is only valid when the Citation Summary is present and every blocking finding satisfies (a) its cited excerpt is absent from the cited file, or (b) it is a [MISSING] finding whose scenario keywords are confirmed present in the spec. If any finding is GENUINE, use ACCEPT. If any finding is AMBIGUOUS, use BLOCKED-AMBIGUOUS.
