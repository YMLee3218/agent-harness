...
| `blocked.jsonl` | JSONL (append-only) | `_record_transient` (transient); `_record_blocked` legacy belt (ceiling/transient only) | `is-blocked` unmigrated-legacy fail-safe + `migrate-events` | `cleared_at:null` = open; transient has its own lifecycle. Block authority for the 6 real kinds moved to `events/`; this is no longer the block source |
