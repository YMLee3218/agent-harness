...
|------|--------|------------|---------|-----------|
| `convergence/{phase}__{agent}.json` | JSON | `_record_loop_state` (via `record-verdict-direct` in `run-critic-loop.sh` for critic-spec/test/code/cross; via SubagentStop hook for critic-feature; via `append-review-verdict`/`run-critic-loop.sh` for pr-review — excluded from SubagentStop) | `is-converged` (`run-dev-cycle.sh`, `run-critic-loop.sh`) | Created on first verdict; reset via `reset-milestone`/`reset-pr-review`/`clear-converged`; `converged=true` requires ≥2 consecutive PASSes |
