...
2. Output the review verdict (before the ultrathink audit in step 3) as a **literal raw line** — not inside a code block, not described in prose; the exact bytes must appear in your output. Use exactly the format injected by `run-critic-loop.sh` into this session's prompt:
   `<!-- review-verdict: {nonce} PASS -->` or `<!-- review-verdict: {nonce} FAIL -->`
   where `{nonce}` is the UUID printed in the prompt. Do NOT write "I output the marker" or refer to "the marker above" — actually print the line. `run-critic-loop.sh` captures the *last* nonce-anchored marker and records the verdict via `append-review-verdict`. Do NOT call `append-review-verdict` directly — `run-critic-loop.sh` must own the final recorded verdict to support REJECT-PASS and ACCEPT-OVERRIDE overrides (the REJECT-PASS and ACCEPT-OVERRIDE overrides in step 3 below each emit an additional marker; `run-critic-loop.sh` captures the *last* occurrence so overrides win). The nonce prevents verdict spoofing via doc citations of the marker format.
