...
2. Output the review verdict (before the ultrathink audit in step 3) in exactly the format injected by `run-critic-loop.sh` into this session's prompt:
   ...
   where `{nonce}` is the UUID printed in the prompt. `run-critic-loop.sh` captures the *last* nonce-anchored marker and records the verdict via `append-review-verdict`. Do NOT call `append-review-verdict` directly — `run-critic-loop.sh` must own the final recorded verdict to support REJECT-PASS and ACCEPT-OVERRIDE overrides (steps 15–16 below emit additional markers; `run-critic-loop.sh` captures the *last* occurrence so overrides win). The nonce prevents verdict spoofing via doc citations of the marker format.
