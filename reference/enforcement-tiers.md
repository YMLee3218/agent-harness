...
**Note on timeout-prefixed spawn sites**: `timeout`/`gtimeout` exec their first argument as a binary via `execvp`, so they cannot invoke `worker_exec` (a bash shell function). **Broken pattern** (timeout WRAPS the sandbox call): `${TIMEOUT_CMD:+$TIMEOUT_CMD} worker_exec bash` — `timeout` would try to exec the shell function `worker_exec`, which fails. Code using this pattern must use `"${_WORKER_SANDBOX_ARGS[@]}"` array expansion directly, preceded by a `_sandbox_guard` call for fail-closed behaviour: `_sandbox_guard; "${_WORKER_SANDBOX_ARGS[@]}" ${TIMEOUT_CMD:+$TIMEOUT_CMD ...} bash`. All engine (claude/codex) spawns now use this array form centrally inside `run_engine` (`scripts/lib/engine-runner.sh` — the sole builder of engine command lines); callers (`implement-helpers.sh`, `integration-helpers.sh`, `run-critic-loop.sh`, `llm-runner.sh`) invoke `run_engine` rather than splicing the array themselves. **Safe pattern** (timeout INSIDE the sandbox call): `worker_exec ${TIMEOUT_CMD:+$TIMEOUT_CMD ...} bash` — `worker_exec` (shell function) is the outer call; it invokes `sandbox-exec` (a binary) which then execs `timeout` (also a binary); no shell-function-via-execvp issue. The only remaining direct `worker_exec` call site is `run-merge-gate.sh` (the project unit/integration test-command runs, not LLM engines).
