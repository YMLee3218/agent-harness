...
Applies to user-facing output: `AskUserQuestion` text, `ExitPlanMode` summaries, conversation replies, runtime narrative from agents (abort reports, error summaries). Critic verdict outputs are excluded — they are machine-readable and must remain English regardless of conversation language. This covers all six phase-gate critics. `critic-spec/test/code/cross/quality`: Codex review verdict (via `run-critic-loop.sh` → `codex exec`, using `skills/critic-*/SKILL.md` templates or `skills/critic-quality/angles/*.md` for critic-quality fan-out) is machine-readable and stays English; on FAIL, the decision agent (Claude, prompt rendered from `prompts/critic-decision.md`) produces structured AUDIT/FIX-PLAN output that also stays English. `critic-feature` (`skills/critic-feature/SKILL.md`): Claude fork verdict in the same machine-readable HTML comment format.
