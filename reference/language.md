...
Applies to user-facing output: `AskUserQuestion` text, `ExitPlanMode` summaries, conversation replies, runtime narrative from agents (abort reports, error summaries). Critic verdict outputs are excluded — they are machine-readable and must remain English regardless of conversation language. This covers all five phase-gate critics: `agents/critic-{code,spec,test,cross}.md` output is copied verbatim from Codex; `agents/critic-feature.md` output is a Claude fork verdict but uses the same machine-readable HTML comment format.
