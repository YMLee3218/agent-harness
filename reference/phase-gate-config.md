...
Model for the orchestration session spawned by `run-critic-loop.sh`. Default: `opus`. The orchestration session runs the one-shot iteration logic (skill invocation, `record-verdict`, ultrathink audit per `@reference/critics.md §Critic one-shot iteration`). For `critic-spec/test/code/cross/quality`, reviews are executed via `codex exec --dangerously-bypass-approvals-and-sandbox` (worker.sb provides Tier 1 confinement) — codex manages its own model independently of Claude. For `critic-feature`, the review is a Claude fork (`skills/critic-feature/SKILL.md`) whose model resolves from the engine registry (`scripts/lib/engine-registry.sh`: `critic-feature` → claude opus, still honouring `CLAUDE_CRITIC_LOOP_MODEL`). `CLAUDE_CRITIC_LOOP_MODEL` controls only the orchestration session and does not affect the codex review itself. For shell-driven critics, the FAIL decision audit is a separate one-shot Claude invocation (`run_engine --role critic-decision`, model sonnet) whose prompt is rendered from `prompts/critic-decision.md` via `build_decision_prompt` in `scripts/lib/critic-helpers.sh`. Engine and model for every role resolve through `engine-registry.sh` and may be overridden per role with `HARNESS_ENGINE_<ROLE>` / `HARNESS_MODEL_<ROLE>` (ROLE uppercased, `-`→`_`).
