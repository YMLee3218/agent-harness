...
**Exit codes**: 0 = converged; 1 = blocked (BLOCKED or BLOCKED-CEILING marker in plan, or session timeout/NOOP); 2 = BLOCKED-CEILING; 3 = lock contention (another run already active for this plan — wait for it to finish or remove the `.plan.critic.lock` file); any other code is a script failure: write `[BLOCKED] script-failure: {code}` to `## Open Questions` and stop. Running critics manually is not a fallback — it is a protocol violation.
### New milestone
