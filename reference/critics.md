...
If none of the above apply, fix and re-run without stopping. **Autonomous mode behaviour**: all bash/write tools are blocked and the session terminates; the question is sent to the human via Telegram. **Resuming**: once you have resolved the question, **run from a human terminal** (`pretooluse-bash.sh` blocks `clear-marker` on `[BLOCKED-AMBIGUOUS]` — Claude cannot execute this): `bash .claude/scripts/plan-file.sh clear-marker plans/{slug}.md "[BLOCKED-AMBIGUOUS] {agent}"`. Then tell the interactive Claude what decision you made; it will implement remaining changes and restart the autonomous run.
