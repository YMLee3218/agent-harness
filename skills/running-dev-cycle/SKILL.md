...
**If the command exits 0 (`[BLOCKED]`)**: surface its exact output verbatim to the user and stop. Do not retry the dev cycle, do not predict outcomes ("this should pass", "a clean run is expected"), do not spawn a fresh `claude -p` invocation. A `HUMAN_MUST_CLEAR_MARKERS` entry means the human owns the next step — the orchestrator's role is to relay status, not to act past the marker. Then follow `@reference/blocked-guidance.md` to present the block in Korean with root-cause-first recommendations.
