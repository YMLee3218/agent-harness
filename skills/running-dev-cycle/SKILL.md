...
| `review` | PR review interrupted — check `## Open Questions`: if `[CONVERGED] review/pr-review` is present → transition to `green`; otherwise → Apply `@reference/pr-review-loop.md` (do **not** call `reset-pr-review`). |
