...
**If the command exits 0 (`[BLOCKED]`)**: immediately follow `@reference/blocked-guidance.md` to present the block in the conversation language (Korean by default) with root-cause-first recommendations. (`is-blocked` produces no stdout — the block details are in blocked.jsonl; use `plan-file.sh context <plan-file>` to surface the current markers if needed.) Do not retry the dev cycle, do not predict outcomes ("this should pass", "a clean run is expected"), do not spawn a fresh `claude -p` invocation. A `HUMAN_MUST_CLEAR_MARKERS` entry means the human owns the next step — the orchestrator's role is to relay status and guide resolution, not to act past the marker.
