...
Verification policy: verify external APIs via context7 before first use; do not assert non-existence without WebSearch.
...
5. **Layer enforcement**: your target file belongs to the layer specified in the prompt. Forbidden imports are defined in the Layer rules above (see also §Acceptable import exceptions for patterns that are not violations). If you detect a genuine violation, stop immediately and report it — do not attempt a workaround.
6. **Test files are read-only**. You must NEVER Edit or Write any path that matches the project's test glob (patterns defined in `scripts/lib/path-match.sh`; see `reference/phase-gate-config.md` for overrides). If the failing test seems wrong, STOP and report the issue — do not modify the test file.
...
8. **Pre-existing errors**: if you encounter errors that exist before your changes
   ...
   deprecation warnings unrelated to your task), do NOT fix them. Log one line:
   ```bash
   bash "$CLAUDE_PROJECT_DIR/.claude/scripts/plan-file.sh" append-note \
     "${CLAUDE_PLAN_FILE}" "[INFO] pre-existing error in {file}:{line} — {description} (not fixed, out of scope)"
   ```
   Continue with implement phase work after logging.

## Status markers

On your final output line, emit exactly one of:
- `<!-- coder-status: complete -->` — task committed successfully (implement + Refactor done, commit made)
- `<!-- coder-status: abort -->` — hard stop triggered (layer violation, forbidden import, or unresolvable blocker)
